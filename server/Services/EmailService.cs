...
public class EmailService : IEmailService
{
    ...
    public async Task SendPublicContactEmailAsync(string contactName, string contactEmail, string message)
    {
        var apiKey = _config["SendGrid:ApiKey"];
        var fromEmail = _config["SendGrid:FromEmail"];
        var fromName = _config["SendGrid:FromName"] ?? "Eloomen";
        ...
        var from = new EmailAddress(fromEmail, fromName);
        var to = new EmailAddress(adminEmail);
        var subject = $"Public Contact Form Submission from {contactName}";

        var htmlContent = $@"
            <html>
            <body style='font-family: -apple-system, BlinkMacSystemFont, ""Segoe UI"", Roboto, ""Helvetica Neue"", Arial, sans-serif; padding: 20px; background-color: #0f172a; color: #e2e8f0; margin: 0;'>
                <div style='max-width: 600px; margin: 0 auto; background-color: #1e293b; border-radius: 12px; padding: 32px; border: 1px solid #334155;'>
                    <h2 style='color: #f1f5f9; margin-top: 0; font-size: 24px; font-weight: 600;'>New Public Contact Form Submission</h2>
                    
                    <div style='background-color: #0f172a; padding: 20px; border-radius: 8px; margin: 24px 0; border: 1px solid #334155;'>
                        <h3 style='color: #cbd5e1; font-size: 16px; margin-top: 0; margin-bottom: 16px; font-weight: 600;'>Contact Information</h3>
                        <p style='color: #94a3b8; font-size: 14px; margin: 8px 0;'><strong style='color: #cbd5e1;'>Name:</strong> {contactName}</p>
                        <p style='color: #94a3b8; font-size: 14px; margin: 8px 0;'><strong style='color: #cbd5e1;'>Email:</strong> {contactEmail}</p>
                        <p style='color: #94a3b8; font-size: 14px; margin: 8px 0;'><strong style='color: #cbd5e1;'>Source:</strong> Public Homepage</p>
                    </div>
                    ...
                    <p style='color: #94a3b8; font-size: 14px; margin-top: 32px; border-top: 1px solid #334155; padding-top: 20px; line-height: 1.6;'>
                        This message was sent from the public Eloomen contact form on the homepage.
                    </p>
                </div>
            </body>
            </html>";
        ...
        if (!response.IsSuccessStatusCode)
        {
            throw new InvalidOperationException("Failed to send public contact email");
        }
    }
