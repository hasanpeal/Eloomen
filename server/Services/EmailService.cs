...
public class EmailService : IEmailService
{
    ...
    public async Task SendEmailConfirmationAsync(string email, string username, string confirmationCode, string confirmationUrl)
    {
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogError("Failed to send email confirmation. Status: {StatusCode}, Body: {Body}", 
    ...
    public async Task SendDeviceVerificationAsync(string email, string username, string deviceCode, string verificationUrl)
    {
        try
        {
            ...
            {
                var body = await response.Body.ReadAsStringAsync();
                _logger.LogError("Failed to send device verification email. Status: {StatusCode}, Body: {Body}", 
                    response.StatusCode, body);
            }
            else
            {
                _logger.LogInformation("Device verification email sent successfully to {Email}", email);
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error sending device verification email to {Email}", email);
        }
    }
    ...
    public async Task SendPasswordResetAsync(string email, string username, string resetCode, string resetUrl)
    {
        try
        {
            ...
            {
                var body = await response.Body.ReadAsStringAsync();
                _logger.LogError("Failed to send password reset email. Status: {StatusCode}, Body: {Body}", 
                    response.StatusCode, body);
            }
            else
            {
