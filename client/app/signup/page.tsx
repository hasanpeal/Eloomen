  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        const redirectUrl = `/verify-email?email=${encodeURIComponent(formData.email)}`;
        if (inviteToken) {
          router.push(`${redirectUrl}&inviteToken=${encodeURIComponent(inviteToken)}`);
        } else {
        ...
        <div className="bg-slate-800/60 backdrop-blur-md rounded-3xl p-8 border border-slate-700/50 shadow-2xl">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex items-start">
              <input
                ...
                type="checkbox"
                required
                className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-slate-600 rounded mt-1"
              />
              <label
                htmlFor="terms"
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-slate-400">
              ...
              <Link
                href={inviteEmail && inviteToken ? `/login?email=${encodeURIComponent(inviteEmail)}&token=${encodeURIComponent(inviteToken)}` : "/login"}
